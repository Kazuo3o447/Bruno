...
> **Medium-Frequency Bitcoin Trading Bot — Retail-Ready v3.0**
>
...
> ✅ **V3.0 System Stabilization:** Bybit deaktiviert, Binance als stabile Primärquelle, Max Pain Logik entfernt, Config Caching implementiert
...
| **Execution-Börse** | **Binance** (Futures, max 1.0× Leverage) |
| **Daten-Börse (Primär)** | **Binance REST API** (stabile Primärquelle, Bybit deaktiviert 2026-04-06) |
| **Daten-Börse (Fallback)** | **Binance WebSocket** (5s Heartbeat Monitoring) |
| **LLM-Stack** | Deepseek Reasoning API (Cloud) für Post-Trade Analyse & Learning |
| **Sentiment Analysis** | HuggingFace Models (facebook/bart-large-mnli) mit HF_TOKEN |
| **News Source** | CryptoPanic API (ersetzt Google Trends, privacy-first) |
| **API Integration** | BinanceDataClient + Alpha Vantage (Macro) + DeepSeek (Analysis) + HuggingFace (Sentiment) |
...
**Primäre Ziele:** Stabilität & Transparenz vor Rendite. **V3.0 API Integration:** Alle API-Keys konfiguriert und validiert, HuggingFace Sentiment Models aktiv, CryptoPanic News Integration. **V3.0 System Stabilization:** Bybit deaktiviert, Binance als stabile Primärquelle, Max Pain Logik entfernt. **V2.2 Retail-Ready:** Echtes CVD, GRSS v3, Adaptive Thresholds. **Paper Trading Lock** für sichere Tests. **Live Marktdaten** von Binance (primär) mit vollständiger API-Integration.
...
- **News Integration:** CryptoPanic API für Echtzeit-Krypto-Nachrichten (ersetzt Google Trends)
...
- **Binance Primary Data Source:** REST API (30s Polling) + WebSocket Streams (6 Streams)
- **Stable Market Data:** Binance als stabile Primärquelle (Bybit deaktiviert 2026-04-06)
- **Sentiment Analysis:** HuggingFace Models mit HF_TOKEN, Zero-Shot Classification für News
- **CryptoPanic News:** API-basierte News-Aggregation (ersetzt Google Trends Browser-Scraping)
...
- **Max Pain Removal:** Max Pain Logik entfernt (2026-04-06), System vereinfacht
...
- **Signal Sources:** Binance API (Primary) + Alpha Vantage (Macro) + HuggingFace (Sentiment) + CryptoPanic (News)
